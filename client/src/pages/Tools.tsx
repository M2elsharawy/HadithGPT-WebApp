...
export default function Tools() {
  ...
  }, [originalAudioUrl, currentAudio?.url]);
  ...
  }, [currentAudio, originalAudioUrl, processedBlobUrl, safeRevoke]);
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      const resp    = await fetch(currentAudio.url, { signal: abortCtrl.signal });
      const blob    = await resp.blob();
      const formData = new FormData();
      formData.append("file", blob, currentAudio.name);
      formData.append("model", "whisper-1");
      if (transcriptLang === "ar") formData.append("language", "ar");
      formData.append("response_format", "json");

      setTranscriptProgress(30);
      ...
      if (!apiResp.ok) {
        throw new Error("خدمة النسخ غير متاحة — تأكد من ضبط OPENAI_API_KEY في الـ server");
      }
        ...
        return (
          <div className={wrapClass}>
            ...
            <div className="bg-teal-50 dark:bg-teal-950/30 border border-teal-200 dark:border-teal-800 rounded-xl p-3 text-xs text-teal-700 dark:text-teal-300 space-y-1">
              <p>• يُشغّل الملف الصوتي ويستمع إليه في الوقت الفعلي</p>
  ...
  const anyBusy  = isCleaningUp || isRemovingSilence || isExportingEQ || isApplyingEffect || isProcessing;
