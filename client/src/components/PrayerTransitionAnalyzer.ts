...
export class PrayerTransitionAnalyzer {
  ...
  static analyze(
    ...
    // ── 5. تحديد "القرآن الطويل" — أطول المقاطع الصوتية ────────────────────
    const quranBlocks = rawSegs.filter(s => s.durationSec >= MIN_QURAN_BLOCK_SEC);
    const firstQuranEnd  = quranBlocks.length > 0 ? quranBlocks[0].endSec   : totalSec;
    const lastQuranStart = quranBlocks.length > 0 ? quranBlocks[quranBlocks.length - 1].startSec : 0;
    ...
    const voicedSegments: VoicedSegment[] = rawSegs.map((seg, idx) => {
      ...
      const isIsolated = silBefore >= MIN_SILENCE_ISOLATION && silAfter >= MIN_SILENCE_ISOLATION;
      const isShort   = seg.durationSec <= MAX_TAKBEER_SEC;
      const isHighEnergy = seg.avgRmsDb >= HIGH_ENERGY_DB;

      let classification: TransitionClass = "review";
      let confidence = 0.3;
      let safeToRemove = false;

      if (seg.durationSec >= MIN_QURAN_BLOCK_SEC) {
        // مقطع طويل = قرآن على الأرجح
        classification = "quran_likely";
        confidence = 0.95;
        safeToRemove = false;

      } else if (posRatio < 0.12 && seg.startSec < firstQuranEnd) {
        // في بداية الملف قبل القرآن = إقامة/افتتاح
        classification = "iqama_or_intro";
        confidence = 0.65;
        safeToRemove = confidence >= MIN_CONFIDENCE_REMOVE && isShort && isHighEnergy;

      } else if (posRatio > 0.88 && seg.startSec > lastQuranStart) {
        // في نهاية الملف بعد القرآن = سلام/خاتمة
        classification = "salam_or_outro";
        confidence = 0.60;
        safeToRemove = confidence >= MIN_CONFIDENCE_REMOVE && isShort;

      } else if (isShort && isIsolated && isHighEnergy) {
        // مقطع قصير + معزول + طاقة عالية = تكبير
        classification = "takbeer_candidate";
        confidence = 0.80;
        safeToRemove = confidence >= MIN_CONFIDENCE_REMOVE;

      } else if (isShort && (silBefore >= 1.5 || silAfter >= 1.5) && !isIsolated) {
        // مقطع قصير بعد/قبل صمت = انتقال (ركوع/سجود)
        classification = "transition_candidate";
        confidence = 0.65;
        safeToRemove = false; // لا نقترح الحذف التلقائي للانتقالات

      } else if (seg.durationSec > MAX_TRANSITION_SEC && seg.durationSec < MIN_QURAN_BLOCK_SEC) {
        // بين القصير والطويل — غير واضح
