...
export class AudioExporter {
  ...
  /**
   * MP3 encoding — يجرب 3 طرق بالترتيب:
  ...
  private static async _encodeMp3(buffer: AudioBuffer, bitrate: Mp3Bitrate): Promise<Blob> {
    ...
    // ── 3. CDN ────────────────────────────────────────────────────────────
    if (!Mp3Encoder) Mp3Encoder = await AudioExporter._tryLamejsCDN();

    if (!Mp3Encoder) throw new Error("lamejs غير متاح في أي مصدر");
  ...
  private static _cdnCache: EncoderCtor | null = null;
  private static async _tryLamejsCDN(): Promise<EncoderCtor | null> {
    if (AudioExporter._cdnCache) return AudioExporter._cdnCache;
    return new Promise(resolve => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const w = window as any;
      if (typeof w.lamejs?.Mp3Encoder === "function") {
        AudioExporter._cdnCache = w.lamejs.Mp3Encoder;
        return resolve(w.lamejs.Mp3Encoder);
      }
      const s = document.createElement("script");
      s.src = "https://cdnjs.cloudflare.com/ajax/libs/lamejs/1.2.1/lame.min.js";
      s.onload = () => {
        const Enc = w.lamejs?.Mp3Encoder ?? w.Mp3Encoder ?? null;
        if (typeof Enc === "function") AudioExporter._cdnCache = Enc;
        resolve(typeof Enc === "function" ? Enc : null);
      };
