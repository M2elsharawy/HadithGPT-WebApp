...
async function startServer() {
  ...
  app.post("/api/whisper", express.raw({ type: () => true, limit: "50mb" }), async (req, res) => {
    try {
      // ── 1. Rate limiting ────────────────────────────────────────────────
      ...
      if (!apiKey) {
        res.status(503).json({ error: "OPENAI_API_KEY غير مضبوط — أضفه في ملف .env" });
        return;
      }

      // ── 4. أعد إرسال الـ multipart مباشرة لـ OpenAI ───────────────────
      // نُمرّر الـ body الخام كما هو مع الـ content-type الأصلي (آمن بعد التحقق)
      ...
      if (!response.ok) {
        ...
        res.status(response.status).json({ error: `OpenAI: ${response.status}`, detail: errText.slice(0, 200) });
        return;
      }
